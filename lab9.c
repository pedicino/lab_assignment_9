...
void displayRecordsInHash(struct HashType *hashTable, int tableSize)
{
    ...
    for (int i = 0; i < tableSize; ++i)
    {
        ...
        if (hashTable->hTableRecs[i] != NULL)
        {
            ...
            struct RecordType *curRec = hashTable->hTableRecs[i];
            printf("Index %d -> ", i);

            while (curRec != NULL)
            {
                printf("%d, %c, %d -> ", curRec->id, curRec->name, curRec->order);
                curRec = curRec->next;
            }
            // last
            printf("NULL\n");
...
int main(void)
{
    ...
    int hashTableSize = 40;
    // initialize a hashTable, use calloc (so everything is assigned to NULL)
    // table itself
    struct HashType *ht = (struct HashType *)calloc(1, sizeof(struct HashType));
    // records
    ht->hTableRecs = (struct RecordType **)calloc(hashTableSize, sizeof(struct RecordType *));

    // for each record in pRecords, insert it into the hash table using the insertRecord function
    for (int i = 0; i < recordSz; ++i)
    {
        insertRecord(ht, &pRecords[i], hashTableSize);
    }

    // call the display records function
    displayRecordsInHash(ht, hashTableSize);
